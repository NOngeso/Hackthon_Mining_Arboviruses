...
def get_options():
    ...
    parser.add_argument('tree', action='store', help='Newick Tree file',
    ...
    parser.add_argument('--format', choices=('png', 'tiff', 'pdf', 'svg'),
                        default='png', help='Output format [Default: png]')
    parser.add_argument(
        '-N', '--skipped-columns', action='store', type=int, default=14,
...
if __name__ == "__main__":
    ...
    plt.hist(roary.sum(axis=1), roary.shape[1], histtype="stepfilled", alpha=.7)
    ...
    plt.savefig(
        os.path.join(options.output_dir, 'pangenome_frequency.%s' % options.format),
        dpi=300)
    plt.clf()
    ...
    with sns.axes_style('whitegrid'):
        ...
        if options.labels:
            ...
            with plt.rc_context({'font.size': fsize}):
                Phylo.draw(
                    ...
                    xlabel=('', ),
                    xlim=(-mdist * 0.1,
                          mdist + mdist * 0.45 - mdist * roary.shape[1] * 0.001),
                    axis=('off', ),
        ...
        plt.savefig(
            os.path.join(options.output_dir, 'pangenome_matrix.%s' % options.format),
            dpi=300)
        plt.clf()
    ...
    a = plt.pie([core, softcore, shell, cloud], labels=[
        ...
        'cloud\n(strains < %d)' % (roary.shape[1] * .15)
                ...
                colors=[(0, 0, 1, float(x) / total)
                        for x in (core, softcore, shell, cloud)], autopct=my_autopct)
    plt.savefig(os.path.join(options.output_dir, 'pangenome_pie.%s' % options.format),
                dpi=300)
    plt.clf()
